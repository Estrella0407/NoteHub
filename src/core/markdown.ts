...
export function parseMarkdown(md: string): string {
  ...
  // Extract math blocks
  md = md.replace(/\$\$([\s\S]*?)\$\$/g, (_, math: string) => 
    stash(`<div class="math-block">${math.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>`)
  );
  ...
  // Inline math
  md = md.replace(/\$([^$\n]+)\$/g, (_, math: string) => 
    stash(`<span class="math-inline">${math.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</span>`)
  );
  ...
  md = md.replace(/(?:^&gt;\s?[^\n]*\n?)+/gm, (match: string) => {
    ...
    return stash(`<blockquote>${content.trim().replace(/\n/g, '<br/>')}</blockquote>`);
  });
