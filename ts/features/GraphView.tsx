...
export default function GraphView() {
  ...
  function nodeColor(d: GraphNode): string {
    if (d.type === 'canvas') return 'var(--accent2, #5bc8f5)';
    if (d.tags.length > 0) return 'var(--accent3, #f7c87b)';
    return 'var(--accent, #7ee8a2)';
  }
  ...
  useEffect(() => {
    ...
    const linkSel = g.append('g').selectAll('line')
      ...
      .attr('stroke', (d: any) => d.linkType === 'wiki' ? 'rgba(126,232,162,0.25)' : 'rgba(247,200,123,0.35)')
    ...
    nodeGroup.append('text')
      ...
      .attr('fill', 'var(--text-main, #e8eaf0)').attr('font-size', '10px')
